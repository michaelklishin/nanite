module Nanite
  ...
  class Mapper
    ...
    def setup_queues
      ...
      amq.queue("heartbeat#{env_prefix}#{@identity}", :exclusive => true).bind(amq.fanout("heartbeat#{env_prefix}")).subscribe{ |ping|
        agent.log.debug "Got heartbeat"
        handle_ping(agent.load_packet(ping))
      }
      amq.queue("mapper#{env_prefix}#{@identity}", :exclusive => true).bind(amq.fanout("registration#{env_prefix}")).subscribe{ |msg|
        agent.log.debug "Got registration"
        register(agent.load_packet(msg))
      }
      amq.queue("#{env_prefix}#{agent.identity}", :exclusive => true).subscribe{ |msg|
